...
module.exports = function(login, accessToken) {
  ...
  var watching;
  if (login) {
    watching = client.watching.listForUser(login, { accessToken: accessToken, maxPages: 10 });
  } else {
    watching = client.watching.listForAuthUser({ accessToken: accessToken, maxPages: 10 });
  }
  return watching
