...
function loadData(login, accessToken) {
  ...
  function makeUrl(pathName) {
    var partial = url.format({ pathname: pathName, query: { login: login, accessToken: accessToken } });
...
module.exports = function(login, accessToken) {
  return users.update(login, { partial: true })
    .then(function() {
      return loadData(login, accessToken);
    })
    .then(function() {
      return users.update(login, { partial: false });
    });
}
