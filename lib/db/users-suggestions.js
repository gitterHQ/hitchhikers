...
function flatten(collatedSuggestions) {
  ...
  return users.map(function(login) {
    ...
    return { login: login, user: collatedSuggestions[login].name, reason: relationships.slice(0,2).join('. ') + '.' };
  });
}
    ...
    .then(function(results) {
      return results.data.map(function(item) {
        ...
        return {
          login: item[0],
          name: item[1],
          reason: 'You\'ve interacted on a lot of common repos, including ' + getUniqueItems(item[2], 2).join(' and ') + '.'
        };
      });
    });
    ...
    .then(function(suggestions) {
      ...
      return getActivitySuggestions(login)
        .then(function(activitySuggestions) {
          ...
          var users = suggestions.reduce(function(memo, suggestion) { memo[suggestion.login] = 1; return memo; }, {});
