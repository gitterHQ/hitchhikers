...
var MAX_DEFAULT_SUGGESTIONS = 10;
        ...
        .then(function(activitySuggestions) {
          ...
          return getCommonSuggestions(login)
            .then(function(commonSuggestions) {
              ...
              // TODO: add speaker suggestions
              var shuffledSpeakers = shuffle(speakers.list);
              var speakerSuggestions = 0;

              while(shuffledSpeakers.length && suggestions.length < MAX_SUGGESTIONS && speakerSuggestions <= MAX_DEFAULT_SUGGESTIONS) {
                var speakerLogin = shuffledSpeakers.pop();
                if (!users[speakerLogin]) {
                  speakerSuggestions++;
                  suggestions.push({
                    login: speakerLogin,
                    name: speakerLogin, // TODO: Use actual name
                    reason: speakerLogin + ' is speaking.',
                    speaker: true
                  });
                }
              }
